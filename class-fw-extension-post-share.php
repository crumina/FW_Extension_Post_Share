...
class FW_Extension_Post_Share extends FW_Extension {
    ...
    public function shortcode( $atts ) {
        ...
        wp_localize_script( 'post-share', 'postShareParams', array(
            'nonce' => wp_create_nonce( 'post-share-nonce' ),
            'ext'   => $this,
            'atts'  => $atts,
        ) );
    ...
    public static function kc_mapping() {
        $shortcodeName = fw_ext( 'post-share' )->get_config( 'shortcodeName' );

        if ( function_exists( 'kc_add_map' ) ) {
            kc_add_map( array(
                $shortcodeName => array(
                    'name'     => esc_html__( 'Post Share', 'post-share' ),
                    'category' => esc_html__( 'Crumina', 'post-share' ),
                    'icon'     => 'kc-post-share-icon',
                    'params'   => array(
